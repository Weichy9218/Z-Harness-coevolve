...
_APIHY_QWEN_API_KEY_ENV = "apihy_API_KEY_qwen"
_APIHY_BASE_URL_ENV = "apihy_BASE_URL"
_DEFAULT_ENV_PROFILES = (
    (_APIHY_QWEN_API_KEY_ENV, _APIHY_BASE_URL_ENV),
    (_OPENROUTER_API_KEY_ENV, _OPENROUTER_BASE_URL_ENV),
)
...
def _pick_default_env_profile() -> tuple[str, str]:
    for api_key_env, base_url_env in _DEFAULT_ENV_PROFILES:
        if os.getenv(api_key_env) and os.getenv(base_url_env):
            return api_key_env, base_url_env
    return _DEFAULT_ENV_PROFILES[0]
...
class OpenRouterNewAPIClient(OpenAISDKClient):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        default_api_key_env, default_base_url_env = _pick_default_env_profile()
        resolved_api_key, self.api_key_source = resolve_client_setting(
            api_key,
            preferred_env=api_key_env,
            fallback_envs=(default_api_key_env, _OPENROUTER_API_KEY_ENV),
        )
        resolved_base_url, self.base_url_source = resolve_client_setting(
            base_url,
            preferred_env=base_url_env,
            fallback_envs=(default_base_url_env, _OPENROUTER_BASE_URL_ENV),
        )
        if not resolved_api_key or not resolved_base_url:
            raise ValueError(
                ...
                f"or one configured env profile such as "
                f"{_APIHY_QWEN_API_KEY_ENV}/{_APIHY_BASE_URL_ENV} or "
                f"{_OPENROUTER_API_KEY_ENV}/{_OPENROUTER_BASE_URL_ENV}"
            )
        super().__init__(
            model=model,
