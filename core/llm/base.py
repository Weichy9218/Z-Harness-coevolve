...
class OpenAISDKClient(BaseLLMClient):
    ...
    def _extract_chat_completion_usage(self, response: Any) -> Dict[str, int]:
        ...
        # Extract cached_tokens from prompt_tokens_details (OpenAI GPT-4.1 format)
        # or directly from usage (some gateways flatten it there).
        cached_tokens = 0
        if usage:
    ...
    def _parse_chat_completion_response(
        ...
        *,
        ...
        if finish_reason == "length" and self._chat_completion_message_has_payload(message):
            pass
